/**
 * MelodIA - ThemeToggle Component
 * Botón conmutador de tema claro/oscuro
 */

import { createElement } from '../utils/domHelpers.js';
import { themeService } from '../services/themeService.js';
import { eventBus } from '../core/eventBus.js';

export class ThemeToggle {
  constructor(containerElement) {
    this.container = containerElement;
    this.render();
    this.bindEvents();
  }

  render() {
    this.container.innerHTML = `
      <button class="theme-toggle-btn" id="theme-toggle-btn" title="Alternar tema claro / oscuro" aria-label="Cambiar tema">
        <!-- Ícono Luna (visible en modo claro) -->
        <svg class="theme-icon-moon" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path>
        </svg>
        <!-- Ícono Sol (visible en modo oscuro) -->
        <svg class="theme-icon-sun" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <circle cx="12" cy="12" r="5"></circle>
          <line x1="12" y1="1" x2="12" y2="3"></line>
          <line x1="12" y1="21" x2="12" y2="23"></line>
          <line x1="4.22" y1="4.22" x2="5.64" y2="5.64"></line>
          <line x1="18.36" y1="18.36" x2="19.78" y2="19.78"></line>
          <line x1="1" y1="12" x2="3" y2="12"></line>
          <line x1="21" y1="12" x2="23" y2="12"></line>
          <line x1="4.22" y1="19.78" x2="5.64" y2="18.36"></line>
          <line x1="18.36" y1="5.64" x2="19.78" y2="4.22"></line>
        </svg>
      </button>
    `;

    const btn = this.container.querySelector('#theme-toggle-btn');
    btn.addEventListener('click', () => {
      themeService.toggleTheme();
    });
  }

  bindEvents() {
    eventBus.on('state:theme', () => {
      // Las clases CSS bajo [data-theme="dark"] manejan la visibilidad de los iconos automáticamente
    });
  }
}
